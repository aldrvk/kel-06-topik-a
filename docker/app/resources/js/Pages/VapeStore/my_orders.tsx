...
export default function MyOrdersPage({ orders }: Props) {
    ...
    return (
        <div className="min-h-screen bg-background flex flex-col">
            <Head title="Riwayat Pesanan - Vape Store" />
            <Navbar />
            ...
            <main className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-10 flex-grow w-full">
                ...
                <div className="mb-8 flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4">
                    <div>
                        ...
                        <p className="text-body-l text-foreground/60 mt-1">Lihat semua pesanan Anda di Venus Vape Store.</p>
                    </div>
                    <Link
                        href="/vape-store"
                ...
                {orders.length === 0 ? (
                    <div className="bg-card border border-border rounded-venus p-12 text-center">
                        ...
                        <p className="text-body-reg text-foreground/50 mt-2">Yuk, lihat koleksi device dan liquid terbaru kami!</p>
                        <Link href="/vape-store" className="mt-5 inline-block bg-super-black text-white px-6 py-3 rounded-full text-label-sm font-bold hover:bg-super-black/80 transition-all">
                            Mulai Belanja
                        </Link>
                    </div>
                ) : (
...
function OrderCard({ order }: { order: Order }) {
    ...
    return (
        <Link
            href={`/vape-store/tracking/${order.order_code}`}
